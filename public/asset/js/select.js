...
$(function () {
    ...
    var optSelected = $('#cusSelectbox').val();
    $('.selectLabel').text($('.options li[rel="'+ optSelected +'"]').text());
    ...
    $(".options li").on('keypress click', function(e) {
        ...
        // troca de moeda na página, recarrega input de valor,
        // carrega a moeda selecionada,
        // carrega as notícias da moeda selecionada
        // adiciona a url e ao histórico a moeda selecionada.
        $(document).find('#quantidade').val(1);
        loadCoin($(this).attr('rel'));
        loadNews($(this).text());
        window.history.pushState("", "", $(this).attr('rel'));
    });
});
